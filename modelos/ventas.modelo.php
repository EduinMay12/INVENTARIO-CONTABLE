...
class ModeloVentas{
	...
	static public function mdlIngresarVenta($tabla, $datos){
		...
		if($stmt->execute()){
			$traeultimoid = ModeloVentas::UltimoFolio($tabla);
	...
	/*=============================================
	ULTIMO FOLIO GUARDADO
	=============================================*/
	static public function UltimoFolio($tabla, $item, $valor){
		try {
			$sql= "Select Max(codigo) as id from $tabla";
			$smlt= conexion::conectar()->prepare($sql);
			$smlt->execute();
			$resultado = $smlt->fetch(PDO::FETCH_COLUMN,0);
			return $resultado;
			
		} catch (Exception $e) {
			return "Exception controlada ".$e;
		}
	}
